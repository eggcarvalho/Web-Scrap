...
- **Arquitetura geral:** aplicativo Laravel moderno (PHP 8.2, Laravel 12) com camadas Services → Repositories → Models. As operações de negócio ficam em `app/Services` e persistência em `app/Repositories` — prefira usar essas camadas ao invés de tocar Models diretamente quando possível.
